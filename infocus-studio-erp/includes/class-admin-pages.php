...
if(sel&&price&&images){
  sel.addEventListener("change",function(){
	...
	public static function render_dashboard() {
		if ( ! Infocus_ERP_Security::current_user_allowed() ) wp_die( 'Not allowed.' );
		$s = Infocus_ERP_Reports::summary();

		$year  = isset( $_GET['cal_year'] ) ? (int) $_GET['cal_year'] : (int) current_time( 'Y' );
		$month = isset( $_GET['cal_month'] ) ? (int) $_GET['cal_month'] : (int) current_time( 'n' );
		$cal_data   = Infocus_ERP_Reports::bookings_for_month( $year, $month );
		$today_day  = ( (int) current_time( 'Y' ) === $year && (int) current_time( 'n' ) === $month ) ? (int) current_time( 'j' ) : 0;
		$first_dow  = (int) date( 'w', strtotime( sprintf( '%04d-%02d-01', $year, $month ) ) );
		$days_in    = (int) date( 't', strtotime( sprintf( '%04d-%02d-01', $year, $month ) ) );
		$prev = array( 'cal_year' => $month === 1 ? $year - 1 : $year, 'cal_month' => $month === 1 ? 12 : $month - 1 );
		$next = array( 'cal_year' => $month === 12 ? $year + 1 : $year, 'cal_month' => $month === 12 ? 1 : $month + 1 );
		?>
		<div class="wrap infocus-erp-wrap infocus-app">
			<div class="infocus-topbar">
				<div>
					<div class="infocus-h1">Studio dashboard</div>
					<div class="infocus-sub"><?php echo esc_html( date_i18n( 'l, j F Y' ) ); ?></div>
				</div>
				<div style="display:flex;gap:8px;">
					<a class="infocus-btn-ghost" href="<?php echo esc_url( admin_url( 'admin.php?page=infocus-erp-bookings' ) ); ?>">New booking</a>
					<a class="infocus-btn-primary" href="<?php echo esc_url( admin_url( 'admin.php?page=infocus-erp-invoices' ) ); ?>">New invoice</a>
				</div>
			</div>

			<?php if ( ! empty( $s['editing_deadlines'] ) ) : ?>
			<div class="infocus-alert-banner">
				<span class="dashicons dashicons-warning"></span>
				<div>
					<div class="infocus-alert-title"><?php echo count( $s['editing_deadlines'] ); ?> booking<?php echo count( $s['editing_deadlines'] ) === 1 ? '' : 's'; ?> need<?php echo count( $s['editing_deadlines'] ) === 1 ? 's' : ''; ?> editing attention</div>
					<div class="infocus-alert-detail">
						<?php
						$parts = array();
						foreach ( $s['editing_deadlines'] as $d ) {
							$parts[] = esc_html( $d['customer_name'] ) . ' — ' . ( $d['is_overdue'] ? 'day ' . $d['days_elapsed'] . ' of 15, overdue' : 'day ' . $d['days_elapsed'] . ' of 15, ' . max( 0, $d['days_left'] ) . ' days left' );
						}
						echo implode( ' &nbsp;·&nbsp; ', $parts );
						?>
					</div>
			...
			<div class="infocus-kpi-row">
				<div class="infocus-kpi-card"><span class="dashicons dashicons-money-alt"></span><div class="infocus-kpi-label">Revenue collected</div><div class="infocus-kpi-num">₹<?php echo number_format( $s['total_revenue_collected'], 0 ); ?></div></div>
				<div class="infocus-kpi-card"><span class="dashicons dashicons-cart"></span><div class="infocus-kpi-label">Total expenses</div><div class="infocus-kpi-num">₹<?php echo number_format( $s['total_expenses'], 0 ); ?></div></div>
				<div class="infocus-kpi-card"><span class="dashicons dashicons-chart-line"></span><div class="infocus-kpi-label">Net profit</div><div class="infocus-kpi-num infocus-num-teal">₹<?php echo number_format( $s['net_profit'], 0 ); ?></div></div>
				<div class="infocus-kpi-card"><span class="dashicons dashicons-clock"></span><div class="infocus-kpi-label">Outstanding</div><div class="infocus-kpi-num infocus-num-crimson">₹<?php echo number_format( $s['total_outstanding'], 0 ); ?></div></div>
			</div>

			<div class="infocus-card-panel" style="margin-bottom:16px;">
				<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;">
					<div style="display:flex;align-items:center;gap:8px;font-weight:500;"><span class="dashicons dashicons-calendar-alt"></span><?php echo esc_html( date_i18n( 'F Y', strtotime( sprintf( '%04d-%02d-01', $year, $month ) ) ) ); ?></div>
					<div style="display:flex;gap:6px;">
						<a class="infocus-cal-nav" href="<?php echo esc_url( add_query_arg( $prev, admin_url( 'admin.php?page=infocus-erp' ) ) ); ?>">&larr;</a>
						<a class="infocus-cal-nav" href="<?php echo esc_url( add_query_arg( $next, admin_url( 'admin.php?page=infocus-erp' ) ) ); ?>">&rarr;</a>
					</div>
				</div>
				<div class="infocus-cal-grid infocus-cal-dow">
					<div>Su</div><div>Mo</div><div>Tu</div><div>We</div><div>Th</div><div>Fr</div><div>Sa</div>
				</div>
				<div class="infocus-cal-grid" id="infocus-cal-days">
					<?php
					for ( $i = 0; $i < $first_dow; $i++ ) echo '<div></div>';
					for ( $d = 1; $d <= $days_in; $d++ ) {
						$has = ! empty( $cal_data[ $d ] );
						$cls = 'infocus-cal-day' . ( $has ? ' has-booking' : '' ) . ( $d === $today_day ? ' is-today' : '' );
						echo '<div class="' . esc_attr( $cls ) . '"' . ( $has ? ' data-day="' . $d . '"' : '' ) . '>' . $d . '</div>';
					}
					?>
				</div>
				<div id="infocus-day-detail" class="infocus-day-detail" style="display:none;"></div>
				<div style="display:flex;gap:16px;margin-top:10px;font-size:12px;color:var(--infocus-slate);">
					<span><span class="infocus-dot" style="background:#131357;"></span> Booking</span>
					<span><span class="infocus-dot" style="background:#D4AF37;border-radius:4px;"></span> Today</span>
				</div>
			</div>

			<script>
			(function(){
				var data = <?php echo wp_json_encode( $cal_data ); ?>;
				var monthLabel = <?php echo wp_json_encode( date_i18n( 'F', strtotime( sprintf( '%04d-%02d-01', $year, $month ) ) ) ); ?>;
				var detail = document.getElementById('infocus-day-detail');
				document.querySelectorAll('.infocus-cal-day.has-booking').forEach(function(cell){
					cell.addEventListener('click', function(){
						document.querySelectorAll('.infocus-cal-day').forEach(function(c){ c.classList.remove('is-selected'); });
						cell.classList.add('is-selected');
						var day = cell.dataset.day;
						var items = data[day] || [];
						var html = '<div style="font-size:12px;font-weight:500;color:var(--infocus-slate);margin-bottom:8px;">' + monthLabel + ' ' + day + ' — ' + items.length + ' booking' + (items.length === 1 ? '' : 's') + '</div>';
						items.forEach(function(b){
							html += '<div style="display:flex;justify-content:space-between;font-size:13px;padding:6px 0;border-bottom:0.5px solid rgba(107,122,143,0.15);"><span>' + b.name + ' — ' + b.service + (b.phone ? ' — ' + b.phone : '') + '</span><span style="color:var(--infocus-slate);">' + b.time + '</span></div>';
						});
			...
			<div class="infocus-card-panel" style="margin-bottom:16px;">
				<h2 style="margin-top:0;">Editing deadlines</h2>
				<?php if ( empty( $s['all_pending_editing'] ) ) : ?>
					<p style="font-size:13px;color:var(--infocus-slate);margin:0;">Nothing in progress yet — this fills in once a client submits their image selection for a booking that isn't marked Delivered. <a href="<?php echo esc_url( admin_url( 'admin.php?page=infocus-erp-pipeline' ) ); ?>">View Editor Pipeline</a></p>
				<?php else : foreach ( $s['all_pending_editing'] as $d ) :
					$bar_color    = $d['is_overdue'] ? 'var(--infocus-crimson)' : ( $d['percent'] >= 66 ? 'var(--infocus-gold)' : 'var(--infocus-teal)' );
					$status_label = $d['is_overdue'] ? 'Overdue' : ( $d['days_left'] . ' day' . ( $d['days_left'] === 1 ? '' : 's' ) . ' left' );
				?>
				<div class="infocus-deadline-row">
					<div class="infocus-deadline-top">
						<span><?php echo esc_html( ( $d['customer_name'] ?: 'Unknown client' ) . ' — ' . $d['service_type'] . ( $d['editor_name'] ? ' — ' . $d['editor_name'] : ' — unassigned' ) ); ?><?php echo $d['is_manual'] ? ' <span style="color:var(--infocus-slate);font-weight:400;">(custom deadline)</span>' : ''; ?></span>
						<span style="color:<?php echo esc_attr( $bar_color ); ?>;font-weight:600;"><?php echo esc_html( $status_label ); ?></span>
					</div>
					<div class="infocus-deadline-bar-track">
						<div class="infocus-deadline-bar-fill" style="width:<?php echo (int) $d['percent']; ?>%;background:<?php echo esc_attr( $bar_color ); ?>;"></div>
					</div>
					<a href="<?php echo esc_url( admin_url( 'admin.php?page=infocus-erp-pipeline' ) ); ?>" style="font-size:11px;color:var(--infocus-slate);text-decoration:none;">Day <?php echo (int) $d['days_elapsed']; ?> of <?php echo (int) $d['days_total']; ?> — adjust in Editor Pipeline</a>
				</div>
				<?php endforeach; endif; ?>
			</div>

			<div class="infocus-erp-columns">
				<div class="infocus-card-panel">
					<h2>Expenses by category</h2>
					<table class="wp-list-table widefat fixed striped">
						<thead><tr><th>Category</th><th>Total (₹)</th></tr></thead>
						<tbody>
						<?php if ( empty( $s['expense_by_category'] ) ) : ?>
							<tr><td colspan="2">No expenses logged yet.</td></tr>
						<?php else : foreach ( $s['expense_by_category'] as $row ) : ?>
							<tr><td><?php echo esc_html( $row['category'] ?: 'Uncategorized' ); ?></td><td>₹<?php echo number_format( $row['total'], 2 ); ?></td></tr>
						<?php endforeach; endif; ?>
						</tbody>
					</table>

					<h2>Editing pipeline — status</h2>
					<table class="wp-list-table widefat fixed striped">
						<thead><tr><th>Status</th><th>Count</th></tr></thead>
						<tbody>
						<?php if ( empty( $s['pipeline_by_status'] ) ) : ?>
							<tr><td colspan="2">No pipeline items yet.</td></tr>
						<?php else : foreach ( $s['pipeline_by_status'] as $row ) : ?>
							<tr><td><?php echo esc_html( $row['status'] ); ?></td><td><?php echo (int) $row['total']; ?></td></tr>
						<?php endforeach; endif; ?>
						</tbody>
					</table>
				</div>
				...
				<div class="infocus-card-panel">
					<h2>Overdue editing / deliveries</h2>
					<table class="wp-list-table widefat fixed striped">
						<thead><tr><th>Booking</th><th>Expected</th><th>Status</th></tr></thead>
						<tbody>
						<?php if ( empty( $s['overdue_pipeline'] ) ) : ?>
							<tr><td colspan="3">Nothing overdue.</td></tr>
						<?php else : foreach ( $s['overdue_pipeline'] as $row ) : ?>
							<tr><td><?php echo esc_html( $row['service_type'] . ' — ' . $row['session_date'] ); ?></td><td><?php echo esc_html( $row['expected_date'] ); ?></td><td><?php echo esc_html( $row['status'] ); ?></td></tr>
						<?php endforeach; endif; ?>
						</tbody>
					</table>
					...
					<table class="wp-list-table widefat fixed striped">
						...
						<tbody>
						<?php if ( empty( $s['unpaid_bookings'] ) ) : ?>
							<tr><td colspan="2">Everyone's paid up.</td></tr>
						<?php else : foreach ( $s['unpaid_bookings'] as $row ) : ?>
							<tr><td><?php echo esc_html( $row['customer_name'] ?: '—' ); ?></td><td>₹<?php echo number_format( $row['balance'], 2 ); ?></td></tr>
						<?php endforeach; endif; ?>
						</tbody>
					</table>
					...
					<table class="wp-list-table widefat fixed striped">
						<thead><tr><th>Customer</th><th>Date</th><th>Service</th></tr></thead>
						<tbody>
						<?php if ( empty( $s['upcoming_bookings'] ) ) : ?>
							<tr><td colspan="3">Nothing scheduled.</td></tr>
						<?php else : foreach ( $s['upcoming_bookings'] as $row ) : ?>
							<tr><td><?php echo esc_html( $row['customer_name'] ?: '—' ); ?></td><td><?php echo esc_html( $row['session_date'] ); ?></td><td><?php echo esc_html( $row['service_type'] ); ?></td></tr>
						<?php endforeach; endif; ?>
						</tbody>
					</table>
				</div>
