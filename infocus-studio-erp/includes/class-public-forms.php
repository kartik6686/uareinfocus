...
class Infocus_ERP_Public_Forms {
	...
	public static function render_inquiry_form() {
		...
		<div class="ifq-wrap">
			<div class="ifc-head">
				<span class="ifc-eyebrow">Let's Talk</span>
				<h2 class="ifc-title">Let's Create Something Beautiful</h2>
				<p class="ifc-sub">Tell us a little about what you're dreaming up.</p>
			</div>

			<?php if ( isset( $_GET['infocus_ok'] ) ) : ?>
				<div class="ifc-notice ifc-notice-ok">Thanks — we've received your details and will be in touch shortly.</div>
			<?php elseif ( isset( $_GET['infocus_err'] ) ) : ?>
				<div class="ifc-notice ifc-notice-err">Something didn't go through. Please check the form and try again.</div>
			<?php endif; ?>
			...
			<div class="ifc-card">
				<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" id="ifq-form">
					...
					<div class="ifc-row">
						<div><label class="ifc-label">Name *</label><input class="ifc-input" type="text" name="name" required></div>
						<div><label class="ifc-label">Phone *</label><input class="ifc-input" type="tel" name="phone" required></div>
					</div>

					<label class="ifc-label">Email (optional)</label>
					<input class="ifc-input" type="email" name="email">

					<label class="ifc-label">Service Interested In</label>
					<div class="ifc-pills" id="ifq-services">
						<?php foreach ( array( 'Maternity', 'Newborn', 'Kids', 'Family', 'Other' ) as $i => $opt ) : ?>
							<button type="button" class="ifc-pill<?php echo $i === 0 ? ' is-active' : ''; ?>" data-value="<?php echo esc_attr( $opt ); ?>"><?php echo esc_html( $opt ); ?></button>
						<?php endforeach; ?>
					</div>
					<input type="hidden" name="service_type" id="ifq-service-type" value="Maternity">

					<label class="ifc-label">Preferred Date</label>
					<div class="ifq-datefield">
						...
						<div class="ifq-cal-panel" id="ifq-cal-panel">
							<div class="ifc-cal-head">
								<button type="button" class="ifc-cal-nav" id="ifq-cal-prev">‹</button>
								<span class="ifc-cal-month" id="ifq-cal-month"></span>
								<button type="button" class="ifc-cal-nav" id="ifq-cal-next">›</button>
							</div>
							<div class="ifc-cal-dow"><span>S</span><span>M</span><span>T</span><span>W</span><span>T</span><span>F</span><span>S</span></div>
							<div class="ifc-cal-grid" id="ifq-cal-grid"></div>
						</div>
					</div>
					<input type="hidden" name="preferred_date" id="ifq-preferred-date" value="">

					<label class="ifc-label">Additional Requirements (optional)</label>
					<textarea class="ifc-textarea" name="additional_requirements" rows="3" placeholder="Any specific requirements, extra touches, or anything else you'd like us to know"></textarea>

					<button type="submit" class="ifc-btn">Send Inquiry</button>
					<p class="ifc-note">We'll get back to you personally, usually within a day.</p>
				</form>
			</div>
		</div>
		</div>
		...
		<script>
		(function(){
			document.querySelectorAll('#ifq-services .ifc-pill').forEach(function(btn){
				btn.addEventListener('click', function(){
					document.querySelectorAll('#ifq-services .ifc-pill').forEach(function(b){ b.classList.remove('is-active'); });
			...
			function render(){
				InfocusPublic.renderMonth({
					...
					prevBtn: prevBtn,
					decorate: function(btn, y, m, d){
						if(selected && selected.y===y && selected.m===m && selected.d===d) btn.classList.add('is-selected');
					},
					onDayClick: function(y, m, d){
						selected = { y:y, m:m, d:d };
						dateText.textContent = MONTHS[m] + ' ' + d + ', ' + y;
						dateText.classList.remove('placeholder');
						hiddenDate.value = y + '-' + InfocusPublic.pad(m+1) + '-' + InfocusPublic.pad(d);
	...
	public static function render_requirements_form() {
		...
		if ( ! $row ) {
			echo '<div class="infocus-public"><div class="ifr-wrap"><div class="ifc-notice ifc-notice-err">This link isn\'t valid. Please check the link your photographer sent you, or ask for a new one.</div></div></div>';
			return ob_get_clean();
		}
		...
		<div class="ifr-wrap">
			<div class="ifc-head">
				<span class="ifc-eyebrow">Let's Plan Your Shoot</span>
				<h2 class="ifc-title">A Few Details to Bring Your Vision to Life</h2>
				<p class="ifc-sub">Everything here helps us prepare exactly what your session needs.</p>
			</div>

			<?php if ( isset( $_GET['infocus_ok'] ) ) : ?>
				<div class="ifc-notice ifc-notice-ok">Thank you — your preferences have been saved.</div>
			<?php endif; ?>
			...
			<?php if ( $booking ) : ?>
				<div class="ifc-details-strip">
					...
					<div class="ifc-details-item"><b>Images Included</b><span><?php echo (int) $booking['included_edits']; ?></span></div>
				</div>
			<?php endif; ?>
			...
			<div class="ifc-card">
				<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
					...
					<label class="ifc-label">1. Location Preference</label>
					<input class="ifc-input" type="text" name="location_preference" value="<?php echo esc_attr( $row['location_preference'] ); ?>" placeholder="Studio, outdoor spot, home, or a specific place">

					<label class="ifc-label">2. Theme or Concept</label>
					<input class="ifc-input" type="text" name="theme" value="<?php echo esc_attr( $row['theme'] ); ?>" placeholder="e.g. fairytale, vintage, minimalist">

					<label class="ifc-label">3. Outfit / Wardrobe Ideas</label>
					<input class="ifc-input" type="text" name="outfit" value="<?php echo esc_attr( $row['outfit'] ); ?>" placeholder="e.g. the fairy dress we discussed">

					<label class="ifc-label">4. Preferred Time / Lighting</label>
					<input class="ifc-input" type="text" name="preferred_time" value="<?php echo esc_attr( $row['preferred_time'] ); ?>" placeholder="e.g. golden hour, sunset, bright daylight">

					<label class="ifc-label">5. Reference Links</label>
					<div class="ifc-hint">Pinterest, Instagram, or any visual references</div>
					<div id="ifr-links-list">
						<?php foreach ( $links as $link ) : ?>
							<div class="ifr-linkrow">
								<input class="ifc-input" type="url" name="reference_links[]" value="<?php echo esc_attr( $link ); ?>" placeholder="Paste a reference link">
								<button type="button" class="ifc-remove-btn">✕</button>
							</div>
						<?php endforeach; ?>
					</div>
					<button type="button" id="ifr-add-link" class="ifc-add-btn">+ Add another link</button>

					<label class="ifc-label" style="margin-top:18px;">6. Anything Else</label>
					<textarea class="ifc-textarea" name="additional_notes" rows="3" placeholder="Any other special requests for the shoot"><?php echo esc_textarea( $row['additional_notes'] ); ?></textarea>

					<button type="submit" class="ifc-btn">Submit Requirements</button>
				</form>
			</div>
		</div>
		</div>
		...
		InfocusPublic.repeatableRows({
			listEl: document.getElementById('ifr-links-list'),
			addBtn: document.getElementById('ifr-add-link'),
			rowHtml: function(){
				return '<div class="ifr-linkrow"><input class="ifc-input" type="url" name="reference_links[]" placeholder="Paste a reference link"><button type="button" class="ifc-remove-btn">✕</button></div>';
			}
		});
		</script>
		<?php
